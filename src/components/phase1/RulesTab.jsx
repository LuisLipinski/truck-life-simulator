  ...
  return (
    <>
      <section className="panel rules-intro" data-tour="rules">
        ...
        {game.id === 'ets2' && <div className="notice-box"><strong>{game.countryFlag} País-sede: {game.countryName} • moeda da carreira: {game.currency}</strong><span>Jornada e descanso seguem a referência operacional europeia. Custo de vida, salário e retenções são calculados na moeda fiscal {game.baseCurrency} de {game.countryName}{game.currency !== game.baseCurrency ? ` e convertidos para ${game.currency} pela cotação fixada em ${game.exchangeRateAsOf}` : ''}. Os cálculos são estimativas de roleplay, não uma folha oficial.</span>{game.financeSources?.length > 0 && <span className="country-source-links">Fontes fiscais: {game.financeSources.map(([label, url], index) => <span key={url}>{index > 0 ? ' • ' : ''}<a href={url} target="_blank" rel="noreferrer">{label}</a></span>)}</span>}{game.currency !== game.baseCurrency && game.exchangeRateSources?.length > 0 && <span className="country-source-links">Fontes cambiais: {game.exchangeRateSources.map(([label, url], index) => <span key={url}>{index > 0 ? ' • ' : ''}<a href={url} target="_blank" rel="noreferrer">{label}</a></span>)}</span>}</div>}
      </section>
